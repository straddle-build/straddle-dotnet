...
/// </summary>
public record class ChargeRefundParams : ParamsBase
{
    ...
    /// <summary>
    /// Refund amount in cents. `null` refunds the full original amount. A value
    /// must be greater than zero and no more than the original charge amount.
    /// </summary>
    public int? Amount
    {
        get
    ...
    /// <summary>
    /// Description for the refund payout. Defaults to a description that identifies
    /// the original charge.
    /// </summary>
    ...
    public string? PaymentDate
    {
        ...
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<string>("payment_date");
        }
    ...
    public ChargeRefundParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public ChargeRefundParams(ChargeRefundParams chargeRefundParams)
        : base(chargeRefundParams)
    {
        this.ID = chargeRefundParams.ID;

        this._rawBodyData = new(chargeRefundParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static ChargeRefundParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData,
        string id
    ...
    public virtual bool Equals(ChargeRefundParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/charges/{0}/refund", this.ID)
        )
        {
