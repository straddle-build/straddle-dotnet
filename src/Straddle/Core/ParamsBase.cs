...
public abstract record class ParamsBase
{
    ...
    static ParamsBase()
    {
        ...
        var headers = new Dictionary<string, string>
        {
            ...
            ["X-Scalar-Arch"] = GetOSArch(),
            ["X-Scalar-Lang"] = "csharp",
            ["X-Scalar-OS"] = GetOS(),
            ["X-Scalar-Package-Version"] = GetPackageVersion(),
            ["X-Scalar-Runtime"] = runtime.Name,
    ...
    internal static void AddDefaultHeaders(HttpRequestMessage request, ClientOptions options)
    {
        ...
        {
            request.Headers.Add("Authorization", string.Format("Bearer {0}", options.Bearer));
        }
        request.Headers.Add(
            "X-Scalar-Timeout",
