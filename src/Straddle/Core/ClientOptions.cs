...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("STRADDLE_BASE_URL") ?? EnvironmentUrl.StraddleApiServer
    );

    /// <summary>
    /// The base URL to use for every request.
    ///
    /// <para>Defaults to the straddle_api_server environment: <see cref="EnvironmentUrl.StraddleApiServer"/></para>
    /// </summary>
    ...
    /// </summary>
    Lazy<string> _bearer = new(() =>
        Environment.GetEnvironmentVariable("BEARER")
        ?? throw new StraddleInvalidDataException(
            string.Format("{0} cannot be null", nameof(Bearer)),
            new ArgumentNullException(nameof(Bearer))
        )
    );
    ...
    {
        readonly get { return _bearer.Value; }
        set { _bearer = new(() => value); }
    }
}
